...
exports.connected = async function (param) {
    ...
    query = query.replace(/&/, '');
    var url = apiRun.getDomain() + '/sodan/v1/uktk/shinki-keizoku?' + query;
    console.log(url);
    apiRun.setURL(url);
...
exports.registed = async function (param, data) {
    ...
    query = query.replace(/&/, '');
    let url = apiRun.getDomain() + '/sodan/v1/uktk/shinki-keizoku?'; // + query;
    apiRun.setURL(url);
    apiRun.setUniqID(param.uniqid);
...
exports.update = async function (param, data) {
    ...
    let url = apiRun.getDomain() + '/sodan/v1/uktk/shinki-keizoku?';
    apiRun.setURL(url);
    apiRun.setUniqID(param.uniqid);
    apiRun.setTraceID(param.traceid);
    apiRun.setInputs(data);
