// @ts-check
// 業務内容マスタ
const Service = require('../../SIENT/MstSyukeiView/Service')
const service = new Service();
const ApiRun = require('../ApiRun');
const apiRun = new ApiRun();
exports.connected = async function (param) {
  apiRun.setQuery(param);
  let url = apiRun.getDomain() + '/sodan/v1/mst/syukei-view/?' + apiRun.getQuery();
  apiRun.setURL(url);
...
exports.registed = async function (query, data) {
  let url = apiRun.getDomain() + '/sodan/v1/mst/syukei-view/';
  console.log(url);
  console.log(data);
  apiRun.setURL(url);
