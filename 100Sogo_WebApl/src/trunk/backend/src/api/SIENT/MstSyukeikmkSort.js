// @ts-check
// 業務内容マスタ
const Service = require('../../SIENT/MstSyukeikmk/Service')
const service = new Service();
const ApiRun = require('../ApiRun');
const apiRun = new ApiRun();
exports.connected = async function (param) {
  apiRun.setQuery(param);
  let url = apiRun.getDomain() + '/sodan/v1/mst/syukeikmk/sort?' + apiRun.getQuery();
  apiRun.setURL(url);
...
exports.registed = async function (query, data) {
  let url = apiRun.getDomain() + '/sodan/v1/mst/syukeikmk/sort';
  console.log(url);
  console.log(data);
  apiRun.setURL(url);
