// @ts-check
// ニーズ整理項目名
const Service = require('../../SIENP/MstAsesTok/Service')
...
exports.registed = async function (query, data) {
  ...
  let url = apiRun.getDomain() + '/sodan/v1/mst/ases-tok/' + apiRun.getQuery();
  console.log(query);
  console.log(data);
  console.log(url);
  apiRun.setURL(url);
...
exports.deleted = async function (query, data) {
  ...
  let url = apiRun.getDomain() + '/sodan/v1/mst/ases-tok/' + createdQuery;
  apiRun.setURL(url);
  apiRun.setUniqID(query.uniqid);
  apiRun.setTraceID(query.traceid);
  apiRun.setInputs(data);
