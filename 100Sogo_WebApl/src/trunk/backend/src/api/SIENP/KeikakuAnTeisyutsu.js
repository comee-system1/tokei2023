// @ts-check
const Service = require('../../SIENP/KeikakuAnTeisyutsu/Service');
...
exports.connected = async function (param) {
    ...
    query = query.replace(/&/, '');
    var url = apiRun.getDomain() + '/sodan/v1/keikakuan/teisyutsu?' + query;
    apiRun.setURL(url);
...
exports.registed = async function (query, data) {
    ...
    var url = apiRun.getDomain() + '/sodan/v1/keikakuan/teisyutsu?' + newquery;
    apiRun.setURL(url);
...
exports.deleted = async function (query) {
    ...
    let url = apiRun.getDomain() + '/sodan/v1/keikakuan/teisyutsu?' + apiRun.getQuery()
    apiRun.setURL(url);
    apiRun.setUniqID(query.uniqid);
    apiRun.setTraceID(query.traceid);
