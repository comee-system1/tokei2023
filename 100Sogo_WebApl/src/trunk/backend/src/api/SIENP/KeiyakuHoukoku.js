// @ts-check
const Service = require('../../SIENP/KeiyakuHoukoku/Service');
...
exports.connected = async function (param) {
    ...
    query = query.replace(/&/, '');
    var url = apiRun.getDomain() + '/sodan/v1/keiyaku-houkoku?' + query;
    apiRun.setURL(url);
...
exports.registed = async function (query, data) {
    let url = apiRun.getDomain() + '/sodan/v1/keiyaku-houkoku?'
    apiRun.setURL(url);
...
exports.deleted = async function (query) {
    ...
    let url = apiRun.getDomain() + '/sodan/v1/keiyaku-houkoku?' + apiRun.getQuery()
    apiRun.setURL(url);
    apiRun.setUniqID(query.uniqid);
    apiRun.setTraceID(query.traceid);
