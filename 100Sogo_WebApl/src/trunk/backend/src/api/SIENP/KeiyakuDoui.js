// @ts-check
const Service = require('../../SIENP/KeiyakuDoui/Service');
...
exports.connected = async function (param) {
    ...
    query = query.replace(/&/, '');
    var url = apiRun.getDomain() + '/sodan/v1/keikaku/doui?' + query;
    apiRun.setURL(url);
...
exports.registed = async function (query, data) {
    ...
    var url = apiRun.getDomain() + '/sodan/v1/keikaku/doui?' + newquery;
    apiRun.setURL(url);
...
exports.deleted = async function (query) {
    ...
    let url = apiRun.getDomain() + '/sodan/v1/keikaku/doui?' + apiRun.getQuery()
    apiRun.setURL(url);
    apiRun.setUniqID(query.uniqid);
    apiRun.setTraceID(query.traceid);
